function [] = Benchmark2testFu (PathDir, SaveDir_Fu)
...
%     pan = [];
    ...
    % parfor i = 1:NumImgs
     for i = 1:NumImgSum
        ...
        %截取和联结lms
        lms = double(imgData.MS_Up);        
  
        %截取和联结ms
        ms = double(imgData.MS);         
  
        %截取和联结pan
        pan = double(imgData.Pan);         
...
%         SaveName = fullfile(SaveDir,loadMatPath_strsplit_char,listing(i).name); %保存的mat文件名还用原文件名
        ...
        SaveName = fullfile(SaveDir_Fu,listing(i).name); %保存的mat文件名还用原文件名，不用二级目录      
        if ~exist(SaveDir_Fu,'dir')
            mkdir(SaveDir_Fu)
        end
        
        save(SaveName, 'lms', 'ms', 'pan', 'Paras' ); % training.mat testing.mat, 'Paras'
