function [] = Benchmark2testFuDiySize (PathDir, panSize, SaveDir,Num)
    PanSize = 1024; %设置原多光谱的边长尺寸
    % panSize = 64; %设置新多光谱的边长尺寸
    SaveDir  = fullfile(SaveDir,['Test_Fu',num2str(panSize)]);
    % 得到新图像边长尺寸
    lmsSize = panSize; 
    msSize = panSize/4;
    ...
    for i=1:length(a)
        ...
        for j=1:n
            ...
            % MS_Up = double(MS_Up);
            % lms = MS_Up((j-1)*gtSize+1:(j-1)*gtSize+gtSize,(k-1)*gtSize+1:(k-1)*gtSize+gtSize,:);
            
            %依次截取一小块作为符合像素要求的图像，暂时命名img_tem
            MS_Up = double(MS_Up);
            lms = MS_Up((j-1)*lmsSize+1:(j-1)*lmsSize+lmsSize,(k-1)*lmsSize+1:(k-1)*lmsSize+lmsSize,:);
            

            %依次截取一小块作为符合像素要求的图像，暂时命名img_tem
            MS = double(MS);
            ms = MS((j-1)*msSize+1:(j-1)*msSize+msSize,(k-1)*msSize+1:(k-1)*msSize+msSize,:);
            

            %依次截取一小块作为符合像素要求的图像，暂时命名img_tem
            Pan = double(Pan);
            pan = Pan((j-1)*panSize+1:(j-1)*panSize+panSize,(k-1)*panSize+1:(k-1)*panSize+panSize,:);
            ...
            save(saveName, 'lms', 'ms', 'pan' ,'Paras');
