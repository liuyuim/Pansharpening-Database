function [] = TifMSPan2Benchmark (MSDataPath,PanDataPath,saveDir,SensorName)
addpath(genpath('.\Toolbox\'));

    % 在当前二级目录处理每一个Tif
    MSlist = dir([MSDataPath,'**/*.tif']) ;
    Panlist = dir([PanDataPath,'**/*.tif']) ;
...
%         SensorName = pathList{i}(PathLen+1:end-15);
        %读取多光谱影像
        [MS,R_MS] = readgeoraster([MSDataPath, MSlist(i).name]);
...
% fprintf(formatSpec, i, NumPatchs);
        ...
        suffix = find('.'== MSlist(i).name); %寻找后缀名前面的标志"。"
        imname = MSlist(i).name(1:suffix-1); 
