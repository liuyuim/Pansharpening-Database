    ...
    for i, ms_path in enumerate(tqdm(input_ms_path)):
        ...
        for j in range(j_iters):
            for k in range(k_iters):
                ...
                if np.sum(ms_data != 0) >= 0.8 * int(ms_data.shape[0] * ms_data.shape[1] * ms_data.shape[2]):
                # 将裁剪得到的多光谱、全色和上采样多光谱影像保存到不同的文件中，并为每个裁剪块添加唯一的编号。
                    ...
                    save_lr_batch_path = os.path.join(output_path_ms, f'{data_index}.tif')
                    save_hr_batch_path = os.path.join(output_path_pan, f'{data_index}.tif')
                    save_up_batch_path = os.path.join(output_path_msup, f'{data_index}.tif')
                    ...
                    for band_index in range(lr_couts):
                        ...
                        # single_band_lr_path = os.path.join(output_path_ms1b, f'{i}_MSband{band_index + 1}_{data_index}.tif')
                        single_band_lr_path = os.path.join(output_path_ms1b, f'{data_index}.tif')
                    ...
                    for band_index in range(lr_couts):
                        ...
                        # single_band_up_path = os.path.join(output_path_msup1b, f'{i}_MSUpband{band_index + 1}_{data_index}.tif')
                        single_band_up_path = os.path.join(output_path_msup1b, f'{data_index}.tif')
...
# TifClip_5qu1.main(input_data_path) # 文件每隔5个取1个
