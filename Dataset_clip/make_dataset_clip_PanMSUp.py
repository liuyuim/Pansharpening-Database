'''
改编自 xyc19970716\Deep-Learning-PanSharpening-main\data

cd/d E:\LiuYu\FusionEvaluateExperiment\dataset_clip
conda activate LYpy3.8pt1.9.0
python make_dataset_clip_PanMSup.py
python make_dataset_clip.py
...
def write_tiff(output_file,array_data,rows,cols,counts,geo,proj):
    ...
    if 'int8' or 'int16' in array_data.dtype.name:
...
def write_tiff_MS1b(output_file,array_data,rows,cols,counts,geo,proj):
...
input_ms_path = glob.glob(os.path.join(input_data_path, "ms_up", "*.TIF"))#r'./ms.tif' "ms""pan"是文件夹名
...
# 2304 is for scale 6
clip_size = 1024 # pan尺寸 1024#256#128#50 #112
scale = 1 # 6
isRandom = False
align_by_geo = True
...
save_path = os.path.join(input_data_path, 'TifClipPanMSUp1024')
...
save_lr_path = os.path.join(save_path, 'MSUp')
save_lr_path_1 = os.path.join(save_path, 'MSUp1')
save_lr_path_2 = os.path.join(save_path, 'MSUp2')
save_lr_path_3 = os.path.join(save_path, 'MSUp3')
save_lr_path_4 = os.path.join(save_path, 'MSUp4')

if not os.path.exists(save_hr_path):
    os.makedirs(save_hr_path)
if not os.path.exists(save_lr_path):
    os.makedirs(save_lr_path)
if not os.path.exists(save_lr_path_1):
    os.makedirs(save_lr_path_1)
if not os.path.exists(save_lr_path_2):
    os.makedirs(save_lr_path_2)
if not os.path.exists(save_lr_path_3):
    os.makedirs(save_lr_path_3)
if not os.path.exists(save_lr_path_4):
    os.makedirs(save_lr_path_4)
...
for i, ms_path in enumerate(input_ms_path):
    ...
    j_iters = lr_rows // (clip_size // scale) # (clip_size // scale) 是 ms 
    ...
    for j in range(j_iters):
        for k in range(k_iters):
            ...
            ms_data1 = lr_array_data[0,h_s:h_e, w_s:w_e].astype(np.uint16)
            ms_data2 = lr_array_data[1,h_s:h_e, w_s:w_e].astype(np.uint16)
            ms_data3 = lr_array_data[2,h_s:h_e, w_s:w_e].astype(np.uint16)
            ms_data4 = lr_array_data[3,h_s:h_e, w_s:w_e].astype(np.uint16)
            ...
            if np.sum(ms_data!=0) >= int(ms_data.shape[0] * ms_data.shape[1] * ms_data.shape[2]):
                ...
                save_lr_batch_path1 = os.path.join(save_lr_path_1, '{}.tif'.format(data_index))
                save_lr_batch_path2 = os.path.join(save_lr_path_2, '{}.tif'.format(data_index))
                save_lr_batch_path3 = os.path.join(save_lr_path_3, '{}.tif'.format(data_index))
                save_lr_batch_path4 = os.path.join(save_lr_path_4, '{}.tif'.format(data_index))
                ...
                write_tiff_MS1b(save_lr_batch_path1, ms_data1, size, size, lr_couts, tuple(lr_b_geo), lr_proj)  
                write_tiff_MS1b(save_lr_batch_path2, ms_data2, size, size, lr_couts, tuple(lr_b_geo), lr_proj)  
                write_tiff_MS1b(save_lr_batch_path3, ms_data3, size, size, lr_couts, tuple(lr_b_geo), lr_proj)  
                write_tiff_MS1b(save_lr_batch_path4, ms_data4, size, size, lr_couts, tuple(lr_b_geo), lr_proj)  
